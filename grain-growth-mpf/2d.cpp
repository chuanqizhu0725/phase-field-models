...
start:;
    ...
    if ((((int)(time1) % 2000) == 0))
    {
        cout << "hello, I started one step!" << time1 << "\n";

        FILE *stream; //ストリームのポインタ設定
        char buffer[30];
        sprintf(buffer, "data/test%d.csv", int(time1));
        ...
        for (j = 0; j <= ndm; j++)
        {
            ...
            for (n1 = 1; n1 <= n00h[i][j]; n1++)
            {
                ...
                for (n2 = 1; n2 <= n00h[i][j]; n2++)
                {
                    ...
                    {
                        kk = m00h[n3][i][j];
                        sum1 += 0.5 * (aij[ii][kk] - aij[jj][kk]) * (ph[kk][ip][j] + ph[kk][im][j] + ph[kk][i][jp] + ph[kk][i][jm] - 4.0 * ph[kk][i][j]) + (wij[ii][kk] - wij[jj][kk]) * ph[kk][i][j]; //[式(4.31)の一部]
                    }
                    pddtt += -2.0 * tij[ii][jj] / double(n00h[i][j]) * (sum1 - 8.0 / PI * eij[ii][jj] * sqrt(ph[ii][i][j] * ph[jj][i][j]));
