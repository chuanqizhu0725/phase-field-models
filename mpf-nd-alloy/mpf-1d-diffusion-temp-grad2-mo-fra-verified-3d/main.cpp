...
double conlr[NDXL][NDYL][NDZL], conlr2[NDXL][NDYL][NDZL];
...
int main(void)
{
    ...
    c0 = sum0 / NDX / NDY / NDZ; // should be equal to cl
    for (i = 0; i <= ndmx; i++)
    {
        for (j = 0; j <= ndmy; j++)
        {
...
#pragma omp parallel num_threads(NTH)
    {
        ...
        double conlrizp, conlrizm, con0izp;
    ...
    start:;
        ...
        if ((istep % pstep == 0) && (th_id == 0))
        {
            ...
            cout << "The nominal concentration is: " << c0 << endl;
            // ****** YZ *******
...
#pragma omp barrier
        ...
        if ((istep % int(dtimeL)) == 0)
        {
            for (iz = startl; iz <= endL; iz++)
            {
                for (ix = 0; ix <= ndmxl; ix++)
                {
                    for (iy = 0; iy <= ndmyl; iy++)
                    {
                        ixp = ix + 1;
                        ixm = ix - 1;
                        iyp = iy + 1;
                        ...
                        {
                            conlrizp = conlr[ix][iy][izm];
                        }
                        if (iz == 0)
                        {
                            conlrizm = cont[ix][iy][ndmz];
                        }
                        else
                        {
                            conlrizm = conlr[ix][iy][izm];
                        }
                        conlr2[ix][iy][iz] = conlr[ix][iy][iz] + dtimeL * Dl * (conlr[ixm][iy][iz] + conlr[ixp][iy][iz] + conlr[ix][iym][iz] + conlr[ix][iyp][iz] + conlrizm + conlrizp - 6.0 * conlr[ix][iy][iz]) / (dxL * dxL);
                    }
                }
            }
            for (iz = startl; iz <= endL; iz++)
            {
                for (ix = 0; ix <= ndmx; ix++)
                {
                    for (iy = 0; iy <= ndmy; iy++)
                    {
                        conlr[ix][iy][iz] = conlr2[ix][iy][iz];
                    }
                }
            }
        }
            ...
            if (intpos > mid)
            {
                ...
                for (iz = (ndmz - dist + 1); iz <= ndmz; iz++)
                {
                    for (ix = 0; ix <= ndmx; ix++)
                    {
                        for (iy = 0; iy <= ndmy; iy++)
                        {
                            ...
                            // cont
                            cont[ix][iy][iz] = conlr[ix][iy][iz - ndmz + dist];
                            // phi
                            phi[0][ix][iy][iz] = 1.0;
                            phi[1][ix][iy][iz] = 0.0;
                            phi[2][ix][iy][iz] = 0.0;
                ...
                for (iz = 0; iz <= (ndmz - dist); iz++)
                {
                    for (ix = 0; ix <= ndmx; ix++)
                    {
                        for (iy = 0; iy <= ndmy; iy++)
                        {
                            conlr[ix][iy][iz] = conlr2[ix][iy][iz + dist];
                        }
                    }
                }
                for (iz = (ndmz - dist + 1); iz <= ndmz; iz++)
...
void datasave(int step)
{
    ...
    //         if (k <= ndmz)
    //         {
    //             fprintf(streamc, "%e   ", cont[NDX / 2][j][k]);
    //             fprintf(streamc, "\n");
    //         }
    //         else
    //         {
    //             fprintf(streamc, "%e   ", conlr[NDX / 2][j][k - ndmz]);
    //             fprintf(streamc, "\n");
    //         }
    //     }
    // }
