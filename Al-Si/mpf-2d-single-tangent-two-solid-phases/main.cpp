...
start:;
    ...
    for (i = 0; i <= ndm; i++)
    {
        for (j = 0; j <= ndm; j++)
        {
            ...
            dev01 = 0.25 * ((phi[0][ip][j] - phi[0][im][j]) * (con0[ip][j] - con0[im][j]) + (phi[0][i][jp] - phi[0][i][jm]) * (con0[i][jp] - con0[i][jm])) / dx / dx;
            dev02 = phi[0][i][j] * (con0[ip][j] + con0[im][j] + con0[i][jp] + con0[i][jm] - 4.0 * con0[i][j]) / dx / dx;

            dev11 = 0.25 * ((phi[1][ip][j] - phi[1][im][j]) * (con1[ip][j] - con1[im][j]) + (phi[1][i][jp] - phi[1][i][jm]) * (con1[i][jp] - con1[i][jm])) / dx / dx;
            dev12 = phi[1][i][j] * (con1[ip][j] + con1[im][j] + con1[i][jp] + con1[i][jm] - 4.0 * con1[i][j]) / dx / dx;

            dev21 = 0.25 * ((phi[2][ip][j] - phi[2][im][j]) * (con2[ip][j] - con2[im][j]) + (phi[2][i][jp] - phi[2][i][jm]) * (con2[i][jp] - con2[i][jm])) / dx / dx;
            dev22 = phi[2][i][j] * (con2[ip][j] + con2[im][j] + con2[i][jp] + con2[i][jm] - 4.0 * con2[i][j]) / dx / dx;
        ...
        for (j = 0; j <= ndm; j++)
        {
            ...
            for (n1 = 1; n1 <= phiNum[i][j]; n1++)
            {
                ...
                for (n2 = 1; n2 <= phiNum[i][j]; n2++)
                {
                    ...
                    if (ii % 2 == 1 && jj == 0)
                    {
                        dF = F10 * ((con0[i][j] - ce) * ml1 + Te - temp);
                    }
                    else if (ii == 0 && jj % 2 == 1)
                    {
                        dF = -F10 * ((con0[i][j] - ce) * ml1 + Te - temp);
                    ...
                    {
                        dF = F20 * ((con0[i][j] - ce) * ml2 + Te - temp);
                    }
                    else if (ii == 0 && jj % 2 == 0)
                    {
                        dF = -F20 * ((con0[i][j] - ce) * ml2 + Te - temp);
