...
#define NDX 80 //差分計算における計算領域一辺の分割数
#define NDY 80
#define NDZ 10
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i <= ndmx; i++)
    {
        for (j = 0; j <= ndmy; j++)
        {
            for (l = 0; l <= ndmz; l++)
            {
                if (i >= NDX * 7 / 8 && j < NDY / 2)
                {
                    phi[1][i][j][l] = 1.0;
                    phi[2][i][j][l] = 0.0;
                    phi[3][i][j][l] = 0.0;
                }
                else if (i >= NDX * 7 / 8 && j >= NDY / 2)
                {
                    phi[2][i][j][l] = 1.0;
                    phi[1][i][j][l] = 0.0;
                    phi[3][i][j][l] = 0.0;
            ...
            for (l = 0; l <= ndmz; l++)
            {
                ...
                for (n1 = 1; n1 <= phiNum[i][j][l]; n1++)
                {
                    ...
                    for (n2 = 1; n2 <= phiNum[i][j][l]; n2++)
                    {
                        ...
                        pddtt += -2.0 * mij[ii][jj] / (double)phiNum[i][j][l] * (sum1 - 8.0 / PI * fij[ii][jj] * (0.5 * i * i / NDX / NDX) * sqrt(phi[ii][i][j][l] * phi[jj][i][j][l]));
